...
主页使用本地字体和远程 HarmonyOS Sans。为减少首屏加载体积，`public/font/Pacifico-Regular.ttf` 是压缩后的 Logo 字体，只覆盖当前站点需要的字符。

如果你要更换 Logo 文案，遇到字母缺失或回退字体，可以用同目录的 `Pacifico-Regular-all.ttf` 替换压缩版，再按需重新压缩。
